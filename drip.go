...
func (cc *ConfiguredClient) create(howMany int) (net.IP, error) {
	if howMany > cc.MainConfig.MaxHosts {
		log.Fatalf("For safety measures, you cannot build more than %d hosts at a time.", cc.MainConfig.MaxHosts)
	}

	dropletName := fmt.Sprintf("%s-%s-%s", cc.MainConfig.HostnamePrefix, *cluster, uuid.New()[:6])
